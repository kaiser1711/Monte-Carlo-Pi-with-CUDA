...
__global__ void picount(Count *totals, int seq_iter) {
	...
	// Computation loop
	for (int i = 0; i < seq_iter; i += 4) {
        float4 x = curand_uniform4(&rng);
        float4 y = curand_uniform4(&rng);
        counter[threadIdx.x] += (x.x * x.x + y.x * y.x <= 1.0f) +
                       (x.y * x.y + y.y * y.y <= 1.0f) +
                       (x.z * x.z + y.z * y.z <= 1.0f) +
                       (x.w * x.w + y.w * y.w <= 1.0f);
    }
